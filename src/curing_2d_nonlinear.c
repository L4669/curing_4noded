...
//const int MAX_ELEMENT = 800, MAX_NODE = 861;
const int MAX_ELEMENT = 400, MAX_NODE = 441;
//const int MAX_ELEMENT = 3200, MAX_NODE = 3321;
...
int main()
{
	...
	char *fname = "./ext/ACTUAL_PROBLEM_20x20_HM_HALF_GEOM.hmascii";
	...
	gsl_vector_set_all(CD_global_E, 1e-20);
...
/* TRANSIENT SOLVER BEGIN */
	...
	double w = 1, T_inf_old = 25+273, dT_inf;
	FILE *fp1;
	...
	while(time <= maxTime)
	{
		...
		dT_inf = (T_inf-T_inf_old)/delta_t;

		gsl_matrix_set_zero(K_global_NM);
		gsl_matrix_set_zero(M_global_NM);
		gsl_vector_set_zero(F_global_N_const);
		...
		global_neumann_bc(nb_array_1, K_global_NM, F_global_N_const, node_arr, ele_arr,
				T_inf, h1, neu_edges);
		global_neumann_bc(nb_array_2, K_global_NM, F_global_N_const, node_arr, ele_arr,
				T_inf, h2, neu_edges);
		...
		if(time == 0)
		{
			gsl_vector_memcpy(F_global_N, F_global_N_const);
            global_heat_source_matrix(node_arr, ele_arr, F_global_N, 
					CD_global_E, Null, Null, T_global_N, Null);
		}
		...
		while(1)
        {
            ...
            global_heat_source_matrix(node_arr, ele_arr, F_global_N_i, 
					CD_global_E_i, CDR_global_E_i, F_global_N_im1, T_global_N_i, T_global_N);
			
			//global_neumann_bc_2(nb_array_1, F_global_N_im1, node_arr, ele_arr, 
        	//	h1, neu_edges, dT_inf, T_global_N_i, T_global_N);

			//global_neumann_bc_2(nb_array_2, F_global_N_im1, node_arr, ele_arr, 
        	//	h2, neu_edges, dT_inf, T_global_N_i, T_global_N);
            ...
            status = gsl_linalg_BiCGStab(dR_dT, dT, R, 1e-7);
			if(status < 0)
			{
        ...
        printf("%g %g %d %g %g\n", time/60, gsl_vector_get(T_global_N_i, 11)-273, 
				itr, maxResidual, gsl_vector_get(CD_global_E_i, 11));
        fprintf(fp1, "%g %g %d %g %g\n", time/60, gsl_vector_get(T_global_N_i, 11)-273					,itr, maxResidual, gsl_vector_get(CD_global_E_i, 11));
		fflush(fp1);
		...
		gsl_vector_memcpy(F_global_N, F_global_N_i);
        time += delta_t;
        maxResidual = 1;
		itr = 1;
	}
